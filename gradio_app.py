        ...
        with (
            ...
            if pc_cond is not None:
                ...
                if isinstance(pc_cond, list):
                    cond_tensor = torch.tensor(pc_cond).float().view(-1, 6)
                    xyz = cond_tensor[:, :3]
                    color_rgb = cond_tensor[:, 3:]
                elif isinstance(pc_cond, dict):
                    xyz = torch.tensor(pc_cond["positions"]).float()
                    color_rgb = torch.tensor(pc_cond["colors"]).float()
                else:
                    xyz = torch.tensor(pc_cond.vertices).float()
                    color_rgb = (
                        torch.tensor(pc_cond.colors[:, :3]).float() / 255.0
                    )
